...
class VotingCog(commands.Cog):
    ...
    async def setup(self, interaction: discord.Interaction):
        ...
        try:
            ...
            final_embed.add_field(
                name="Next Steps",
                value="Users can now use `/vote` to vote for your server! Use `/recent_votes` to see latest voters.",
                inline=False
            )
    ...
    async def vote(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text=f"You have {config['question_time']} seconds to answer")
        ...
        try:
            response = await self.bot.wait_for('message', check=check, timeout=config['question_time'])
            
            try:
                user_answer = int(response.content.strip())
                if user_answer == correct_answer:
                    self.count_vote(interaction.user.id, interaction.user.name, interaction.guild_id)
                    self.set_user_cooldown(interaction.user.id, interaction.guild_id)
                    ...
                    success_embed.add_field(name="Correct Answer", value=f"```\n{correct_answer}\n```", inline=False)
                ...
                await response.reply("❌ Please provide a valid number!")
                
        except TimeoutError:
            await interaction.followup.send("⏱️ Time's up! Your answer wasn't submitted in time.", ephemeral=True)
    ...
    async def leaderboard(self, interaction: discord.Interaction):
        ...
        leaderboard_embed = discord.Embed(
            title="🏆 Live Vote Leaderboard",
            description="Most recent voters (updates every 30 seconds)",
        ...
        for idx, vote in enumerate(sorted_votes, 1):
            ...
            leaderboard_embed.add_field(
                name=f"#{idx} {vote['username']}",
                value=f"Voted **{relative_time}**",
                inline=False
            )
        
        message = await interaction.response.send_message(embed=leaderboard_embed)
